...
public class MainActivity extends Activity implements OnSeekBarChangeListener {
	...
	public static final String BROADCAST_SEEKBAR = "com.example.lection6.sendseekbar";
	Intent intent;
	...
	private void updateUI(Intent serviceIntent) {
		...
		if (songEnded == 1) buttonPlayStop.setBackgroundResource(R.drawable.playbuttonsm);
	}
	...
	private void playAudio() {
		...
		if (isOnline) {
			...
			registerReceiver(broadcastReceiver, new IntentFilter(AudioService.BROADCAST_ACTION));
			;
	...
	private void checkConnectivity() {
		...
		if (cm.getNetworkInfo(ConnectivityManager.TYPE_MOBILE).isConnectedOrConnecting() || cm.getNetworkInfo(ConnectivityManager.TYPE_WIFI).isConnectedOrConnecting())	isOnline = true;
		else isOnline = false;
	}
	...
	protected void onResume() {
		if (!mBufferBroadcastIsRegistered) {
			registerReceiver(broadcastBufferReceiver, new IntentFilter(AudioService.BROADCAST_BUFFER));
