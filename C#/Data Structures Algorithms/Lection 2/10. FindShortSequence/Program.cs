    ...
    public class ShortestSequence
    {
        ...
        public static void Main()
        {
            ...
            while (queue.Count > 0)
            {
                ...
                if (queue.Count > 0) CreateNewNode(currentNode, OperationTwo(currentNode.Value), queue);

                if (queue.Count > 0) CreateNewNode(currentNode, OperationMultiply(currentNode.Value), queue);
            }
        }
        ...
        private static void CreateNewNode(Node<int> currentNode, int newValue, Queue<Node<int>> queue)
        {
            ...
            else if (nodeOne.Value < M) queue.Enqueue(nodeOne);  
        }
        ...
        private static void Print(Node<int> nodeOne)
        {
            ...
            {
                if (sb.Length == 0) sb.Append(result.Pop());
                else sb.Append(" -> " + result.Pop());
            }
