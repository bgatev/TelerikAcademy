...
namespace TreeTraversal
{
    public class Program
    {
            ...
            foreach (var node in nodes)
            {
                if (node.Children.Count > 0)
                {
                    ...
                    BFS(node, n =>
                    {
                        ...
                        if (sum > searchedSum) return false;
            ...
            foreach (var node in nodes)
            {
                if (node.Children.Count > 0)
                {
                    ...
                    DFS(node, n =>
                    {
                        ...
                        if (sum == searchedSum) treePaths.Add(String.Join(" -> ", stack.Reverse()));
        ...
        private static List<Node<int>> FindAllLeafs(Node<int>[] nodes)
        {
            ...
            foreach (var node in nodes)
            {
                if (node.Children.Count == 0) leafs.Add(node);
            }
        ...
        private static List<Node<int>> FindAllMiddleNodes(Node<int>[] nodes)
        {
            ...
            foreach (var node in nodes)
            {
                if (node.HasParent && node.Children.Count > 0) middleNodes.Add(node);
            }
        ...
        public static void Main(string[] args)
        {
            ...
            foreach (var leaf in leafs) Console.Write("{0}, ", leaf.Value);
            Console.WriteLine();
