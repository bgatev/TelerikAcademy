window.onload = function() {
    ...
    var data = parseData(inputData.familyMembers);
...
function initializeLayer() {
    ...
    layer.on("mousemove", function() { document.body.style.cursor = 'pointer'; });

    layer.on("mouseout", function() { document.body.style.cursor = 'default'; });
...
function parseData(data) {
    ...
    for (var i = 0; i < JsonToObject.length; i++) {
        var node = new Node(JsonToObject[i].mother, JsonToObject[i].father, JsonToObject[i].children);
    ...
    for (var name in dictionary) {
        ...
        for (var i = 0; i < node.children.length; i++) {
            ...
            if (dictionary[childName] && !(childName instanceof Node)) {
                ...
                if (dictionary[childName].mother === childName) dictionary[childName].isFemale = true;
            }
...
function drawFamilyTree(layer, root) {
    ...
    while (queue.length > 0) {
        ...
        for (var i = 0; i < node.children.length; i++) {
            ...
            var paddingRight = 0;
            if (node.children.length > 1) paddingRight = widthStep * (node.children.length - 1);
...
function drawSubTree(layer, root) {
    if (root.father) addFigureToLayer(layer, root.posX, root.level, root.father || "", 5);
    if (root.mother) addFigureToLayer(layer, root.posX + widthStep, root.level, root.mother || "", 17);
    ...
    for (var i = 0; i < root.children.length; i++) {
        ...
        if (child.father === null || child.isFemale) x += widthStep;
...
Node.prototype.hasChild = function(name) {
    for (var i = 0; i < this.children.length; i++) {
        ...
        if (child.mother === name || child.father === name) return true;
    }
};