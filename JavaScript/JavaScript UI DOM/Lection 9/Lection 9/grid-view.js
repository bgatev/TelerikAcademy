...
var controls = (function () {
    ...
    function buildGridView(selector, data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].setOfCourses.length > 0) row.addHeader;
        }
    ...
    function Row(cells) {
        ...
        this.getData = function () { return nestedTable.getData(); }

        this.render = function () {
            var fragment = document.createDocumentFragment();
            var item = document.createElement("tr");

            for (var i = 0; i < this.cells.length; i++) item.appendChild(this.cells[i].render());
        ...
        this.getNestedGrid = function () { return nestedTable; }
    ...
    function NestedGrid() {
        ...
        this.getData = function () { return coursesData; }
        ...
        this.render = function () {
            ...
            for (var i = 0; i < header.length; i++) headerElements.appendChild(header[i].render());
            nestedTable.appendChild(headerElements);

            var tableBody = document.createElement("tbody");

            for (var i = 0; i < rows.length; i++) tableBody.appendChild(rows[i].render());
        ...
        grid.addEventListener("click", function (ev) {
            ...
            if (clickedItem.parentNode instanceof HTMLTableRowElement) {
                ...
                if (rowElement.nextSibling.className == "nested") {
                    ...
                    if (nestedRow.style.display == "none") nestedRow.style.display = "";
                    else nestedRow.style.display = "none";
                }
            }
...
function School(name, location, numberOfCourses, speciality, setOfCourses) {
    ...
    this.setOfCourses = [];
    if (setOfCourses) this.setOfCourses = setOfCourses;
}
...
function Course(title, startDate, totalStudents, setOfStudents) {
    ...
    this.setOfStudents = [];
    if (setOfStudents) this.setOfStudents = setOfStudents;
}